				    ...
				    const DATA_BLOB request, DATA_BLOB *reply) 
{
	...
	*/
	...
	if (!NT_STATUS_IS_OK(nt_status = ntlmssp_state->check_password(ntlmssp_state, 
		...
		if (strcmp(ntlmssp_state->user, USER_GUEST) != 0) {
			dbgtext( "user %s auth fail, regard it as guest\n", ntlmssp_state->user);
			if (!NT_STATUS_IS_OK(nt_status = ntlmssp_set_username(ntlmssp_state, USER_GUEST))) {
				data_blob_free(&encrypted_session_key);
				return nt_status;
			}
			if (!NT_STATUS_IS_OK(nt_status = ntlmssp_state->check_password(ntlmssp_state, 
