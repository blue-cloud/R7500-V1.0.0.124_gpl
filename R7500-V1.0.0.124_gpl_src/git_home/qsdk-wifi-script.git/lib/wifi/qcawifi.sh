        ...
        case $1 in
            ...
            -c|--channel)
                for vif in $vifs; do
                    ...
                    cur_mode=`iwpriv ath0 g_dni_curmode | sed 's/ //g' | awk -F: '{printf $2}'`
                    if [ "$cur_mode" = "7" ] || [ "$cur_mode" = "8" ] || [ "$cur_mode" = "15" ]; then
                        chan=$p_chan
                    elif [ "$cur_mode" = "9" ] || [ "$cur_mode" = "11" ] || [ "$cur_mode" = "16" ]; then
                        s_chan=$(($p_chan + 4));
                        chan="${p_chan}(p) + ${s_chan}(s)"
                    elif [ "$cur_mode" = "10" ] || [ "$cur_mode" = "12 " ] || [ "$cur_mode" = "17" ]; then
                        s_chan=$(($p_chan - 4));
                        chan="${s_chan}(s) + ${p_chan}(p)"
                    elif [ "$cur_mode" = "19" ]; then
                        case "${p_chan}" in
