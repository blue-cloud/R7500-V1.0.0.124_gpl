...
set_quantenna_config_ip() {
    ...
    for vif in $vifs; do
	    ...
	    brctl hairpin "$br_if" $config_interface on
        fi
...
qtn_hw_init() {
    ...
    [ "$mode" = "ap" -a "$op_mode" = "Access point" ] || [ "$mode" = "sta" -a "$op_mode" = "Station" ] || {
        ...
        qt_call 0 set_enable_mu 0
    }
    qt_call 0 rfenable 1
    qtn_vlan_setting "enable"
    ...
    if [ "$region" = "us" ]; then
        qt_call 0 set_chan_pri_inactive 116 0
        qt_call 0 set_chan_pri_inactive 120 0
        qt_call 0 set_chan_pri_inactive 124 0
        qt_call 0 set_chan_pri_inactive 128 0
    fi
    ...
    [ "$region" = "eu" ] && qt_call 0 overwrite_country_code eu fr
...
qtn_vlan_setting(){
    ...
    local main_tag=100
    local guest_tag=200
    config_get_bool lan_restricted "wla_guest" lan_restricted
    ...
    if [ -n "$lan_restricted" ]; then
         if [ "$state" = "enable" ]; then
             ...
             qt_call 1 vlan_config unbind "$guest_tag"
             qt_call 0 vlan_config unbind "$main_tag"
...
enable_qtwifi() {
    ...
    for vif in $vifs; do
        ...
        [ "$confidx" = "0" ] && {
            ...
            [ -n "$wmm" ] && qt_call $confidx set_option wmm "$wmm" 
        }

        # enable pmf for wifi cer
...
wps_setting()
{
    ...
    qt_call 0 set_wps_param ap_pin_fail_method auto_lockdown
    qt_call 0 set_wps_param ap_setup_locked "$ap_setup_locked"
    qt_call 0 set_wps_param ap_pin_fail_method default
    ...
    while [ "$#" -gt "0" ]; do
        case $1 in
            ...
            -c|--channel)
                ...
                [ -z "$isap" ] && continue
                p_chan=`qcsapi_sockrpc get_channel wifi0`
                bw=`qcsapi_sockrpc get_bw wifi0`
                if [ "$bw" = "20" ]; then
                    chan=$p_chan
                elif [ "$bw" = "40" ]; then
                    case "${p_chan}" in
    ...
    while [ "$#" -gt "0" ]; do
        case $1 in
            -c|--client_pin)
                    ...
                    ACTION="SET_STATE" METHOD="PIN" /lib/wifi/5g-wps-process &
                shift 2
                ;;
            -p|--pbc_start)
                    [ -f "/tmp/wps_start_by_2g" ] && {
                    ...
                    ACTION="SET_STATE" METHOD="PBC" /lib/wifi/5g-wps-process &
                shift
                ;;
            -s|--wps_stop)
...
wla_lan_restricted_access()
{
    ...
    # enable hairpin mode
    brctl hairpin "$br_if" "$main_if" on
    brctl hairpin "$br_if" "$guest_if" on
