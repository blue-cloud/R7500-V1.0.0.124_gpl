...
get_all_board_params(char *p)
{
	...
	if (board->bc_name < (char *)RUBY_DRAM_BEGIN) {
		/*FIXME: to be removed when the cause of bug 14826 is fixed */
		printk("Board configuration structure content: 0x%p \n", board);
		print_hex_dump(KERN_EMERG, "Board data ", DUMP_PREFIX_ADDRESS,
				16, 1, board, sizeof(*board), 1);
	}
