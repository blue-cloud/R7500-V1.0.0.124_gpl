...
static void muc_death_run_helper(void)
{
	int ret;
	char *argv[] = { "/bin/sh", "/scripts/muc_crash_save_dmesg", NULL };
	char *envp[] = {
		"HOME=/",
		"TERM=linux",
		"PATH=/sbin:/bin:/usr/sbin:/usr/bin", NULL };

	if (get_flash_size() <= FLASH_64KB) {
		printk(KERN_WARNING "%s: Flash size <= 64KB, disabling MuC crash log\n",
				__FUNCTION__);
		return;
	}
	...
	if (ret != 0) {
		printk(KERN_ERR "%s: running %s %s failed, return code %d\n",
				__FUNCTION__, argv[0], argv[1], ret);
	}
}
