...
int qdrv_fwt_sw_sync_setup(struct qdrv_wlan *qw);
uint32_t qdrv_fetch_timestamp(struct qdrv_wlan *qw, uint16_t index);
int qdrv_notify_entry_add(struct qdrv_wlan *qw, uint16_t index);
...
/*	channel number		frequency 40M upper / lower*/
	{1,		2412,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{2,		2417,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{3,		2422,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{4,		2427,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{5,		2432,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{6,		2437,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{7,		2442,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{8,		2447,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{9,		2452,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{10,		2457,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{11,		2462,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{12,		2467,		0/*No 40M mode  re*/,	0,    0,	0,	0},
	{13,		2472,		0/*No 40M mode  re*/,	0,    0,	0,	0},
};
		...
		struct ieee80211vap *vap, const uint8_t *macaddr, const uint8_t tmp_node)
{
	...
	if (vap->iv_opmode == IEEE80211_M_HOSTAP
			&& qv->qv_vlanmode == QVLAN_MODE_DYNAMIC) {
		qn->vlan_id = QVLAN_DEF_PVID; /* By default put a STA in VLAN 1 */
	} else {
		qn->vlan_id = QVLAN_VID_ALL;
	}
		...
		const struct ieee80211_channel *chan)
{
	...
	if ((ic->ic_opmode == IEEE80211_M_STA) && ic->ic_bss_bw) {
		max_bw = ic->ic_bss_bw;
	}
	...
	int fast_switch, int is_req_chan)
{
	...
	if (fast_switch && ((chan->ic_flags & IEEE80211_CHAN_DFS) ||
...
static void qdrv_wlan_80211_dissasoc(struct ieee80211_node *ni)
{
	struct ieee80211vap *vap = ni->ni_vap;
	struct ieee80211com *ic = vap->iv_ic;
...
static void qdrv_wlan_register_node(struct ieee80211_node *ni)
{
	...
	fwt_sw_register_node(ni->ni_node_idx, qv->qv_vap_idx);
#endif
}
...
static void qdrv_wlan_unregister_node(struct ieee80211_node *ni)
{
	...
	fwt_sw_unregister_node(ni->ni_node_idx, qv->qv_vap_idx);
#endif
}
...
static void qdrv_wlan_80211_beacon_update(struct ieee80211vap *vap)
{
	...
	if (qv->bc_skb[1])
		dev_kfree_skb_any(qv->bc_skb[1]);

	qv->bc_skb[1] = qv->bc_skb[0];

	qv->bc_skb[0] = ieee80211_beacon_alloc(ni, &qv->qv_boff);
	if (qv->bc_skb[0] == NULL) {
		return;
	}
	ieee80211_beacon_update(ni, &qv->qv_boff, qv->bc_skb[0], 0);

	if (!(ioctl = vnet_alloc_ioctl(qv)) ||
			!(bc_args = qdrv_hostlink_alloc_coherent(NULL, sizeof(*bc_args),
			&args_dma, GFP_DMA | GFP_ATOMIC))) {
		DBGPRINTF_E("Failed to allocate BEACON UPDATE message\n");
		dev_kfree_skb_any(qv->bc_skb[0]);
		qv->bc_skb[0] = 0;
		vnet_free_ioctl(ioctl);
		return;
	}
	...
	bc_args->pkt_data = (u32) dma_map_single(NULL,
			qv->bc_skb[0]->data, qv->bc_skb[0]->len, DMA_TO_DEVICE);

	bc_args->bo_tim = bc_args->pkt_data + (qv->qv_boff.bo_tim - qv->bc_skb[0]->data);
	bc_args->bo_tim_len = qv->qv_boff.bo_tim_len;

	if ((ic->ic_flags & IEEE80211_F_DOTH) && (ic->ic_flags_ext & IEEE80211_FEXT_TPC))
		bc_args->bo_tpc_rep = bc_args->pkt_data + (qv->qv_boff.bo_tpc_rep - qv->bc_skb[0]->data);
	else
		bc_args->bo_tpc_rep = 0;
	bc_args->bo_chanswitch = bc_args->pkt_data + (qv->qv_boff.bo_chanswitch - qv->bc_skb[0]->data);
	...
	ioctl->ioctl_arg1 = qv->devid;
	ioctl->ioctl_arg2 = qv->bc_skb[0]->len | (1 << 16);
	ioctl->ioctl_argp = args_dma;
...
static void qdrv_wlan_80211_set_cap_bw(struct ieee80211_node *ni, int bw)
{
	...
	if (bw == BW_HT20) {
		...
		if (IS_IEEE80211_VHT_ENABLED(ic)) {
			...
			printk(KERN_INFO "BW %d cannot be configured in current phymode\n", bw);
		}
		...
		printk(KERN_INFO "BW %d is not valid\n", bw);
	}
	...
	printk(KERN_INFO "QDRV: PHY mode %d\n", ic->ic_phymode);
}
...
static void qdrv_wlan_80211_set_cap_sgi(struct ieee80211_node *ni, int sgi)
{
	...
	if (sgi) {
		/* since we need to use Legacy OFDM rates for BACK in HT20 mode
		 * we cannot set SGI for HT20, because SGI + Legacy rate == invalid combination
		 * and Envy STB cannot handle it too
		 */
		if (ic->ic_htcap.cap & IEEE80211_HTCAP_C_CHWIDTH40) {
			ic->ic_htcap.cap |= (IEEE80211_HTCAP_C_SHORTGI40);
			ic->ic_htcap.cap &= ~IEEE80211_HTCAP_C_SHORTGI20;
		} else {
			/* should not enable SGI on HT20, see the reason above */
			/* Reset SGI flag for HT40. This might be enbled for HT40 */
			ic->ic_htcap.cap &= ~(IEEE80211_HTCAP_C_SHORTGI40 |
						IEEE80211_HTCAP_C_SHORTGI20);
		}
	} else {
...
static void qdrv_wlan_80211_set_vht_mcsset(struct ieee80211com *ic)
{
	...
	ic->ic_vhtcap.rxmcsmap = qdrv_wlan_80211_vhtmcs_map(min(max_vht_rx_nss_cap, ic->ic_vht_nss_cap),
						IEEE80211_VHT_MCS_0_9);
	ic->ic_vhtcap.txmcsmap = qdrv_wlan_80211_vhtmcs_map(min(max_vht_tx_nss_cap, ic->ic_vht_nss_cap),
						IEEE80211_VHT_MCS_0_9);
}
...
static void qdrv_wlan_80211_set_11ac_mode(struct ieee80211com *ic, int vht)
{
	...
	if (vht == QTN_11NAC_ENABLE) {
		...
		if ( (ic->ic_phymode == IEEE80211_MODE_11AC_VHT80PM) || (ic->ic_phymode == IEEE80211_MODE_11AC_VHT40PM) ) {
			ic->ic_phymode = IEEE80211_MODE_11NA_HT40PM;
	...
	int value, unsigned char *data, int len)
{
	...
	case IEEE80211_PARAM_VHT_NSS_CAP:
		qdrv_wlan_80211_set_vht_mcsset(ic);
		break;
 ...
 * with HS 2.0 vaps, which do not support 4-address stations or static ARP.
 */
int qdrv_proxy_arp(struct ieee80211vap *iv, struct qdrv_wlan *qw, uint8_t *data_start)
{
	...
	arp = (struct ether_arp *)data_start;

	/* Verify Gratuitous ARP */
	if (arp->ea_hdr.ar_op == __constant_htons(ARPOP_REPLY) ||
			arp->ea_hdr.ar_op == __constant_htons(ARPOP_REQUEST)) {
		if (!memcmp(arp->arp_spa, arp->arp_tpa, sizeof(arp->arp_spa))) {
			return 1;
		}
	}

	if (arp->ea_hdr.ar_op == __constant_htons(ARPOP_REQUEST)) {
		s_ipaddr = get_unaligned((u32 *)&arp->arp_spa);
		t_ipaddr = get_unaligned((u32 *)&arp->arp_tpa);
		...
		ni = ieee80211_find_node_by_ip_addr(iv, t_ipaddr);
		if (ni) {
...
void qdrv_wlan_80211_stats(struct ieee80211com *ic, struct iw_statistics *is)
{
	 ...
	 */
	rssi = (int8_t)iw_stats_log->stat_buffs[curr_index].rx_phy_stats.last_rssi_all;
	noise = (int8_t)iw_stats_log->stat_buffs[curr_index].rx_phy_stats.hw_noise;
	is->qual.qual  = rssi;
	is->qual.noise = noise;
	is->qual.level = ((((int)rssi + noise) <= 0) ? ((int)rssi + noise) : 0);
...
static int qdrv_wlan_80211_cfg_ht(struct ieee80211com *ic)
{
	...
	if (!ieee80211_swfeat_is_supported(SWFEAT_ID_2X2, 0) &&
		!ieee80211_swfeat_is_supported(SWFEAT_ID_2X4, 0) &&
			!ieee80211_swfeat_is_supported(SWFEAT_ID_4X4, 0)) {
...
static int qdrv_wlan_80211_cfg_vht(struct ieee80211com *ic)
{
	...
	if (ic->ic_mu_enable) {
		if (ic->ic_opmode == IEEE80211_M_STA) {
			ic->ic_vhtcap.cap_flags |= IEEE80211_VHTCAP_C_MU_BEAM_FORMEE_CAP;
		} else if (ic->ic_opmode == IEEE80211_M_HOSTAP) {
			ic->ic_vhtcap.cap_flags |= IEEE80211_VHTCAP_C_MU_BEAM_FORMER_CAP;
		}
	}

	ic->ic_vhtcap.maxmpdu = IEEE80211_VHTCAP_MAX_MPDU_11454;
	ic->ic_vhtcap.chanwidth = IEEE80211_VHTCAP_CW_80M_ONLY ;
	ic->ic_vhtcap.rxstbc = IEEE80211_VHTCAP_RX_STBC_UPTO_1;
	ic->ic_vhtcap.maxampduexp = IEEE80211_VHTCAP_MAX_A_MPDU_1048575;
	ic->ic_vhtcap.lnkadptcap = IEEE80211_VHTCAP_LNKADAPTCAP_BOTH;

	if (ieee80211_swfeat_is_supported(SWFEAT_ID_2X2, 0)) {
		ic->ic_vht_nss_cap = IEEE80211_VHT_NSS2;
		ic->ic_vhtcap.bfstscap = IEEE80211_VHTCAP_RX_STS_2;
		ic->ic_vhtcap.numsounding = IEEE80211_VHTCAP_SNDDIM_2;
	} else if (ieee80211_swfeat_is_supported(SWFEAT_ID_2X4, 0)) {
		ic->ic_vht_nss_cap = IEEE80211_VHT_NSS4;
		ic->ic_vhtcap.bfstscap = IEEE80211_VHTCAP_RX_STS_4;
		ic->ic_vhtcap.numsounding = IEEE80211_VHTCAP_SNDDIM_2;
	} else if (ieee80211_swfeat_is_supported(SWFEAT_ID_4X4, 0)) {
		ic->ic_vht_nss_cap = IEEE80211_VHT_NSS4;
		ic->ic_vhtcap.bfstscap = IEEE80211_VHTCAP_RX_STS_4;
		ic->ic_vhtcap.numsounding = IEEE80211_VHTCAP_SNDDIM_4;
	} else {
                DBGPRINTF_E("%s: stream mode is not valid\n", __func__);
                return -1;
        }
	...
	ic->ic_vhtcap.rxlgimaxrate = 0;	/* revisit */
	ic->ic_vhtcap.txlgimaxrate = 0;	/* revisit */

	ic->ic_vhtop.chanwidth = IEEE80211_VHTOP_CHAN_WIDTH_80MHZ;
	ic->ic_vhtop.centerfreq0 = 0;	/* revisit */
	ic->ic_vhtop.centerfreq1 = 0;	/* Not supported in current BBIC4 hardware */

	ic->ic_vhtop.basicvhtmcsnssset = qdrv_wlan_80211_vhtmcs_map(IEEE80211_VHT_NSS1,
...
static int qdrv_wlan_80211_init(struct ieee80211com *ic, u8 *mac_addr, u8 rf_chipid)
{
	...
	ic->ic_disassoc = qdrv_wlan_80211_dissasoc;
	...
	if (qdrv_wlan_80211_cfg_vht(ic) != 0)
		return -1;
	...
	ic->ic_ocac.ocac_cfg.ocac_params.wea_duration_secs = IEEE80211_OCAC_DURATION_DEFAULT;
	ic->ic_ocac.ocac_cfg.ocac_params.wea_cac_time_secs = IEEE80211_OCAC_CAC_TIME_DEFAULT;
...
#else
	/* Scan count for short range association issue*/
	ic->ic_pwr_adjust_scancnt = get_bootcfg_scancnt();
#endif
	...
	ic->ic_node_mu_grp_update = ieee80211_node_mu_grp_update;
	ic->ic_mu_grp_qmat_update = qdrv_txbf_mu_grp_qmat_update;
#endif
	...
	ic->ic_txbf_pkt_cnt = QTN_TXBF_TX_CNT_DEF_THRSHLD;
...
{
	qdrv_sch_set_dscp2tid_map(vapid, ip_dscp,listlen, ac);
	return;
}
...
static void qdrv_show_wlan_stats(struct seq_file *s, void *data, u32 num)
{
	...
	seq_printf(s, "  rx_3_br_accel_msdu        : %d\n", qw->rx_stats.rx_3_br_accel_msdu);
...
void qdrv_update_cgq_stats(void *ctx, uint8_t index, uint32_t qlen)
{
	struct qdrv_wlan *qw = (struct qdrv_wlan *)ctx;

	qw->cgq_stats.congest_qlen[index] = qlen;
}
	...
	u32 arg1, u32 arg2)
{
	...
	qdrv_fwt_sw_sync_setup(qw);
	fwt_sw_register_qdrv_cb(qw, (fwt_fetch_timestamp)qdrv_fetch_timestamp,
			(fwt_notify_entry_add)qdrv_notify_entry_add);
...
static void qdrv_wps_button_device_file_create(struct net_device *ndev)
{
...
static void qdrv_polling_wps_button_exit(void)
{
	del_timer(&qdrv_wps_button_timer);
}
...
void qdrv_wps_button_exit(struct net_device *dev)
{
	u8	wps_gpio_pin = 0;
	u8	use_interrupt = 0;
	u8	active_logic = 0;
...
int qdrv_fwt_sw_sync(struct qdrv_wlan *qw)
{
	struct shared_params *sp;
	struct qtn_fwt_sw_params *fwt_sw_lhost;
	int ret;
        uint64_t op2;
        uint64_t op3;

        sp = qtn_mproc_sync_shared_params_get();
        fwt_sw_lhost = sp->fwt_sw_params_lhost;
	ret = qdrv_hostlink_fwt_sw_sync(qw, sp->fwt_sw_params_bus);

	/* accelerate muc2host jiffies calculation */
	op2 = fwt_sw_lhost->muc_jiffies_base;
	op2 *= HZ;

	op3 = jiffies;
	op3 *= fwt_sw_lhost->muc_hz;
        fwt_sw_lhost->m2h_op = op2 - op3;
...
{
	int ret = 0;
	struct qtn_fwt_sw_params *lhost;

        lhost = qw->sp->fwt_sw_params_lhost;
	ret = qdrv_fwt_sw_sync(qw);

	if (!ret) {
		DBGPRINTF(DBG_LL_DEBUG, QDRV_LF_TRACE,
			"lhost_hz=%u muc_hz=%u lhost_jiffies=%lu muc_jiffies=%u\n",
                        HZ,
			lhost->muc_hz,
			jiffies,
			lhost->muc_jiffies_base);
	} else {
		DBGPRINTF(DBG_LL_ERR, QDRV_LF_TRACE, "fwt sw sync fail\n");
	}
 ...
 *   (current muc jiffies - muc jiffies baseline) / muc HZ =
 *   (current lhost jiffies - lhost jiffies baseline) / lhost HZ
 *   actual calculation has optimized
 */
static __inline__ uint32_t qdrv_convert_jiffies_muc2host(struct qtn_fwt_sw_params *p, uint64_t muc_j)
{
	uint64_t op;

	op = muc_j * HZ - p->m2h_op;
	do_div(op, p->muc_hz);
...
/*
 * this function gets MUC time diff for specified entry
 * there is still possibility of getting incorrect result
 * since MuC jiffies will overflow
 */
uint32_t qdrv_fetch_timestamp(struct qdrv_wlan *qw, uint16_t index)
{
	if (index < TOPAZ_FWT_HW_TOTAL_ENTRIES) {
		...
		uint64_t muc_j64;
                ...
                lhost = qw->sp->fwt_sw_params_lhost;
                p32 = (uint32_t *)&lhost->muc_fwt_ts_mirror[index];

		muc_l = *p32;
		muc_h = *(p32 + 1);
		if (muc_l == ULONG_MAX && muc_h == ULONG_MAX) {
			return ULONG_MAX;
		} else {
                        muc_j64 = muc_h;
			muc_j64 = ((muc_j64 << 32) | muc_l);
			return qdrv_convert_jiffies_muc2host(lhost, muc_j64);
		}
	} else {
		return ULONG_MAX;
	}
