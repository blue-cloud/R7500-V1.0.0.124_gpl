...
int qdrv_vap_vlan2index_sync(struct qdrv_vap *qv)
{
	...
	if (qv->qv_vlanmode == QVLAN_MODE_MBSS)
		vid = qv->qv_vlan & QVLAN_MASK_VID;
	else
		...
		uint32_t *sub_port_bitmap, int size)
{
	...
	mcast_entry = fwt_db_get_sw_mcast_ff(qv->qv_vap_idx);
	...
	u32 arg1, u32 arg2)
{
	...
	vap->iv_sta_assoc_limit = 0;
	vap->iv_ssid_pri = MIN_PRIORITY_PER_VAP;
	...
	if (stamode || repeater_mode) {
		...
		((struct qdrv_wlan *) mac->data)->ic.ic_opmode = IEEE80211_M_STA;
		...
		((struct qdrv_wlan *) mac->data)->ic.ic_opmode = IEEE80211_M_HOSTAP;
	...
	qv->iv.iv_debug &= ~IEEE80211_MSG_DEBUG;
	qv->iv.iv_debug  = 0;
	qv->qv_vlanmode = QVLAN_MODE_PTHRU;
...
#ifdef CONFIG_QVSP
	if (ic->ic_rf_chipid != CHIPID_DUAL) { /* Disable VSP on RFIC5 */
		if (qw->qvsp == NULL) {
	...
	if (ic->ic_rf_chipid == CHIPID_DUAL && vap->iv_opmode != IEEE80211_M_WDS) {
		/* Disable PPPC and GI probing with RFIC5 */
		ic->ic_setparam(vap->iv_bss, IEEE80211_PARAM_PPPC_SELECT, 0, NULL, 0);
		ic->ic_setparam(vap->iv_bss, IEEE80211_PARAM_GI_SELECT, 0, NULL, 0);

		/* Disable static one-bit auto-correlation on RFIC5 */
		ic->ic_setparam(vap->iv_bss, IEEE80211_PARAM_1BIT_PKT_DETECT, 0, NULL, 0);
...
void qdrv_vap_resource_cleanup(struct qdrv_vap *qv)
{
	...
	for (i = 0; i < QDRV_VAP_BEACON_BUFFERS; i++) {
		if (qv->bc_skb[i] != NULL) {
			dev_kfree_skb_any(qv->bc_skb[i]);
			qv->bc_skb[i] = NULL;
		}
	}
