 ...
 */
static __sram_text int qdrv_accept_data_frame(struct ieee80211vap *vap,
	struct ieee80211_node *ni, int key,
	struct sk_buff *skb, __be16 ether_type)
{
	...
	if (!ni || ((vap->iv_opmode == IEEE80211_M_HOSTAP) && (skb->src_port == 0))) {
		...
		err = "not encrypted";
			...
			&& ether_type == __constant_htons(ETH_P_8021Q)) {
		struct vlan_ethhdr *vlan_eth = (struct vlan_ethhdr *)(skb->data);
		uint8_t vapid = QDRV_WLANID_FROM_DEVID(qv->devid);
		uint8_t pass;
		uint16_t vlan_mode;

		vlan_mode = (qv->qv_vlan >> QVLAN_SHIFT_MODE);
		vid = ntohs(vlan_eth->h_vlan_TCI) & VLAN_VID_MASK;

		if (vlan_mode == QVLAN_MODE_PTHRU)
			pass = qtn_vlan_is_allowed(vlan_bitmap, vid, vapid);
		else if (vlan_mode  == QVLAN_MODE_MBSS)
			pass = (vid == (qv->qv_vlan & QVLAN_MASK_VID));
		else
			pass = (vid == qn->vlan_id);
		...
		struct qdrv_rx_skb_list *skb_rcv_list, uint32_t pseudo_rssi)
{
	...
	if (*qw->sp->vlan_enabled_lhost)
		vlan_mode = QVLAN_MODE(qv->qv_vlan);
	else
		vlan_mode = QVLAN_MODE_PTHRU;

	if (vlan_mode == QVLAN_MODE_DYNAMIC) {
		struct qdrv_node *qn = container_of(ni, struct qdrv_node, qn_node);
		vid = qn->vlan_id;
	} else {
		vid = QVLAN_VID(qv->qv_vlan);
	}

	if (qtn_rx_decap(&wh_copy, skb->data, skb->len, vlan_mode, vid,
					...
					uint8_t *addr, struct iphdr *p_iphdr)
{
	...
	if (igmp_snoop == BR_IGMP_SNOOP_HYBRID && IEEE80211_IS_MULTICAST(addr) &&
			br_fdb_get_hook != NULL &&
			br_fdb_get_hook(dev->br_port->br, skb, addr) == 0)
		return 1;
...
static int qdrv_3addr_br_tqe_accel(struct qdrv_wlan *qw, struct qdrv_vap *qv,
				const struct ether_header *eh, struct sk_buff *skb)
{
	/*
	* Send the packet back to TQE if DA was changed
	* by the qdrv bridge
	*/
	int idx;
	int src_idx;
	...
	if (!is_multicast_ether_addr(eh->ether_dhost)
			&& qv->iv.iv_opmode == IEEE80211_M_STA) {
		idx = fwt_sw_get_index_from_mac_be(eh->ether_shost);
		if (idx < 0)
			return 0;
		src_idx = idx;

		fwt_ent = fwt_db_get_table_entry(idx);
		if (!fwt_ent || !fwt_ent->valid)
			return 0;
		fwt_db_update_timestamp(src_idx);

		idx = fwt_sw_get_index_from_mac_be(eh->ether_dhost);
		if (idx < 0)
			return 0;

		fwt_ent = fwt_db_get_table_entry(idx);
		if (!fwt_ent || !fwt_ent->valid || fwt_ent->out_port == TOPAZ_TQE_LHOST_PORT)
			return 0;
...
#else
static inline int qdrv_3addr_br_tqe_accel(struct qdrv_wlan *qw, struct qdrv_vap *qv,
				const struct ether_header *eh, struct sk_buff *skb)
{
	return 0;
}
...
#define CHECK_VENDOR	do {						\
	...
	if (check_3addr_br) {
		 ...
		 */
		rc = qdrv_rx_set_dest_mac(qw, qv, eh, skb);
		if (qdrv_rx_lncb_should_drop(dev, vap, eh, l3hdr)) {
			...
			if (qdrv_3addr_br_tqe_accel(qw, qv, eh, skb))
				return 1;
		}
	}

	/* Check if node is authorized to receive */
	if (!qdrv_accept_data_frame(vap, ni,
	...
	if (ether_type == __constant_htons(ETH_P_IP)) {
		iphdr_p = l3hdr;
		ip_proto = iphdr_p->protocol;
#ifdef CONFIG_IPV6
		if (iphdr_p->version == 6) {
			ip6hdr_p = (struct ipv6hdr *)iphdr_p;
			iputil_v6_skip_exthdr(ip6hdr_p, sizeof(struct ipv6hdr),
				&ip_proto, (skb->len - ((uint8_t *)ip6hdr_p - skb->data)),
				NULL, NULL);
		}
#endif
	}
	...
	if (vap->proxy_arp) {
		if (ether_type == __constant_htons(ETH_P_ARP)) {
			if (qdrv_proxy_arp(vap, qw, (uint8_t*)l3hdr)) {
				dev_kfree_skb(skb);
				return 1;
			}
#ifdef CONFIG_IPV6
		} else if (ether_type == __constant_htons(ETH_P_IPV6)) {
			if (qdrv_wlan_handle_neigh_msg(vap, qw, (uint8_t*)l3hdr, 0, skb)) {
				dev_kfree_skb(skb);
				return 1;
			}
#endif
