...
void qdrv_pktlogger_netdebug_stats_send(unsigned long data)
{
	...
	trace_ippkt_dropped(TRACE_IPPKT_DROP_RSN_MUC_TX_XATTEMPTS,
				stats->stats_muc_tx.tx_xattempts, 1);
...
static void qdrv_pktlogger_ratedebug_send(unsigned long data)
{
	...
	struct muc_tx_stats *tx_stats = (struct muc_tx_stats *)qw->pktlogger.stats_uc_tx_ptr;
	...
	*mu_rates_read_ptr = 1;
	...
	rate_stats->rate_gen_stats.tx_subframe_success = tx_stats->ampdu_subframe_done;
	rate_stats->rate_gen_stats.tx_subframe_fail = tx_stats->ampdu_subframe_failure;
	rate_stats->rate_gen_stats.tx_mgmt_success = tx_stats->txdone_mgmt;
	rate_stats->rate_gen_stats.tx_hw_retry = tx_stats->tx_hw_retry;
	rate_stats->rate_gen_stats.tx_sw_retry = tx_stats->tx_sw_retry;
...
qdrv_pktlogger_ip_send(void *data, int len, u_int16_t flag_off)
{
	...
	iphdr_p = (struct iphdr *)((void *)etherhdr_p + sizeof(struct ether_header));
	ippayload_p = (void *)((void *)iphdr_p + sizeof(struct iphdr));
