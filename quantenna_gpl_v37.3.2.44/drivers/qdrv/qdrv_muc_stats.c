...
static const char g_header_line[]       = KERN_DEBUG "Tstamp"" SU/MU"" RxPkt"" AMSDU"" RxG""   CRC""  Noise"" TxPkt"" Defers"" Touts"" Retries"" ShPmbl""  LgPmbl"" Scale"" MCS-(TX/RX)""  RSSI / RCPI / EVM\n";
static const char g_stat_comm_fmt[]     = KERN_DEBUG    "%6d"   "%6s"   "%6d"   "%6d" "%4d"   "%6d"    "%7s"   "%6d"    "%7d"   "%6d"     "%8d"    "%7d"     "%8d"   "%6d";
				...
				int show_all_nodes, int csv_format)
{
	...
	if (show_all_nodes) {
		...
		TAILQ_FOREACH(ni, &nt->nt_node, ni_list) {
			for (mu = start[opt]; mu < stop[opt]; mu++) {
#define STAT_ALL_FMT "\t\t%s\tNode " DBGMACVAR "\trxpkts %u txpkts %u\t"
