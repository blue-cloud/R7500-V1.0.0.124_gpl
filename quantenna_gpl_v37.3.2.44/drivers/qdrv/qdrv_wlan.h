...
struct qdrv_node
{
	...
	uint16_t               vlan_id;	/* VLAN to which the node belongs when VAP is in dynamic mode */
};
...
struct qdrv_wlan_tx_stats {
	...
	/**
	 * The number of broadcast or multicast frames transmitted as unicast frames.
	 */
...
struct qdrv_wlan_rx_stats {
	...
	/*
	 * The number of 3-address mode packets forwarded over the fast path.
	 */
	unsigned int rx_3_br_accel_msdu;
...
#else
#define QDRV_WLAN_TX_USE_AUC(qw)	( 0 )
#endif
...
int qdrv_hostlink_fwt_sw_sync(struct qdrv_wlan *qw, struct qtn_fwt_sw_params *bus);
