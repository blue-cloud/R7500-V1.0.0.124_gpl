...
#if defined(TOPAZ_PLATFORM) && !defined(CONFIG_TOPAZ_PCIE_HOST)
...
static int qdrv_command_dumpkcrash(struct device *dev, int argc, char *argv[]);
...
unsigned int g_dbg_test_flags = 0;
...
s_command_table[] =
{
	...
	{ "dumpcrash",	qdrv_command_dumpkcrash },
#if defined(QTN_DEBUG)
...
s_param_table[] =
{
	...
	{
		"test1", P_FL_TYPE_INT, (char *) &g_dbg_test_flags,
		0, sizeof(g_dbg_test_flags)
	},
	{
	...
	{
		"br_vlan_forward", P_FL_TYPE_INT, NULL,
		0, sizeof(u_int32_t)
	},
	{
...
static int qdrv_command_dev_init(struct device *dev, struct qdrv_cb *qcb)
{
	...
	if (get_hardware_revision() >= HARDWARE_REVISION_TOPAZ_A2)
		topaz_tqe_emac_reflect_to(TOPAZ_TQE_LHOST_PORT);
...
static int qdrv_command_start(struct device *dev, int argc, char *argv[])
{
	...
	if (opmode > 0) {
		...
		if (get_hardware_revision() == HARDWARE_REVISION_RUBY_A) {
			DBGPRINTF_E("This image does not support Ruby Rev A\n");
			return -1;
		}
#endif
...
#else
static struct reg_range qdrv_hw_regs[] = {
	{0xE5030000, 0xE503FFFF, "Packet memory"},
	{0xE5040000, 0xE5041FFF, "TCM"},
	{0xE5042000, 0xE50420FF, "Global control"},
	{0xE5050000, 0xE5051FFF, "TX frame processor regs"},
	{0xE5052000, 0xE5052FFF, "RX frame processor regs"},
	{0xE5053000, 0xE5053FFF, "Shared frame processor regs"},
	{0xE6000000, 0xE6000323, "BB Global regs"},
	{0xE6010000, 0xE60100F7, "BB BP regs"},
	{0xE6020000, 0xE602002B, "BB TXVEC regs"},
	{0xE6030000, 0xE6030037, "BB RXVEC regs"},
	{0xE6040000, 0xE604FFFF, "BB SPI0 regs"},
	{0xE6050000, 0xE6050267, "BB MIMO regs"},
	{0xE6090000, 0xE60905F7, "BB TD (1) regs"},
	{0xE6091000, 0xE60914AF, "BB TD (2) regs"},
	{0xE6092000, 0xE60921D3, "BB TD (3) regs"},
	{0xE60A0000, 0xE60A03FF, "BB TD (RFC reg)"},
	{0xE60A1000, 0xE60A133F, "BB TD (RFC W mem 0) regs"},
	{0xE60A2000, 0xE60A233F, "BB TD (RFC W mem 1) regs"},
	{0xE60A3000, 0xE60A333F, "BB TD (RFC W mem 2) regs"},
	{0xE60A4000, 0xE60A433F, "BB TD (RFC W mem 3) regs"},
	...
	{0xE60B3000, 0xE60B313F, "BB TD (gain AG) regs"},
	{0xE60B4000, 0xE60B4063, "BB TD (gain AG) regs"},
	{0xE60F0000, 0xE60F01FB, "BB 11B regs"},
	{0xE6100000, 0xE6107FFF, "BB QMatrix regs"},
	{0xE6200000, 0xE6200FFB, "BB FFT dump registers rx chain 1"},
	{0xE6201000, 0xE6201FFB, "BB FFT dump registers rx chain 2"},
	{0xE6202000, 0xE6202FFB, "BB FFT dump registers rx chain 3"},
...
#else
	char frame_type[8][12] = {"0 prestore","1 manage","2 data","3 A-MSDU","4-A-MPDU","5 beacon","6 probe","7 res"};
#endif
...
#else
	printk("\nframe_type    %s",frame_type[(temp>>4) & 0x7]);
#endif
...
#else
		printk("\tSeqPtr:       %8x",(temp >> 0) & 0xffff);
#endif
...
static int qdrv_command_calcmd(struct device *dev, int argc, char *argv[])
{
	...
	if(cmd[0] == 31) {
		...
		printk("MuC: RF1_TX = %d, RF2_TX = %d\n", num_rf1_tx, num_rf2_tx);
		printk("MuC: RF1_RX = %d, RF2_RX = %d\n", num_rf1_rx, num_rf2_rx);
		...
		if (flag == EXT_TEMPERATURE_SENSOR_REPORT_FLAG) {
			printk("Gain = %d, %d, %d, %d\n",
					(cmd[15] | cmd[16] << 8), (cmd[17]  | cmd[18] << 8), (cmd[20] | cmd[21] << 8), (cmd[22] | cmd[23] << 8));

		} else if (flag == DISABLE_REPORT_FLAG) {
			printk("Power compensation is Disabled\n");
		} else {
			printk("(RF,BB) = (%d, %d), (%d, %d), (%d, %d), (%d, %d)\n",
					cmd[5], cmd[10], cmd[6], cmd[11], cmd[7], cmd[12], cmd[8], cmd[13]);
			printk(" Voltage = %d, %d, %d, %d\n",
					(cmd[15] | cmd[16] << 8), (cmd[17]  | cmd[18] << 8), (cmd[20] | cmd[21] << 8), (cmd[22] | cmd[23] << 8));
		}
		printk("TEMPERATURE_RFIC_EXTERAL= %d.%d\n", temp_cal_13_I, temp_cal_13_P);
		printk("TEMPERATURE_RFIC_INTERAL = %d.%d\n", rfic_temp_int, rfic_temp_frac); //Please do not delete for future use
		...
		nsts = cmd[25];
		format = cmd[27];
		printk("MCS = %d, RX SYMBOL NUM = %d, NSTS = %d, BW = %d, FORMAT = %d  _\n", mcs, num_rx_sym, nsts, bw, format);
#else
		DBGPRINTF(DBG_LL_HIDDEN, QDRV_LF_CALCMD, "MCS = %d, RX SYMBOL NUM = %d  _\n", mcs, num_rx_sym);
#endif

		printk("RX_GAIN = %d  __ (0x%x)\n",  rx_gain, rx_gain);
		...
		if(rssi_flag > 0)	//rssi_flag is 0 for EVM measurement
		{
			...
			DBGPRINTF(DBG_LL_HIDDEN, QDRV_LF_CALCMD,
				...
				rssi[0]/10, ABS(rssi[0])%10 , rssi[1]/10, ABS(rssi[1])%10 , rssi[2]/10, ABS(rssi[2])%10 , rssi[3]/10, ABS(rssi[3])%10);
			else
			DBGPRINTF(DBG_LL_HIDDEN, QDRV_LF_CALCMD,
...
#endif
		{
			...
			DBGPRINTF(DBG_LL_HIDDEN, QDRV_LF_CALCMD,
					...
					evm_int[0], evm_frac[0], evm_int[1], evm_frac[1], evm_int[2], evm_frac[2], evm_int[3], evm_frac[3]);
		}
	} else if(cmd[0] == 48) {
		...
		printk("OUT PD_LEVEL : (%d, %d, %d, %d) / RFIC_TEMP = %d oC\n",
				pd_vol0_reading, pd_vol1_reading, pd_vol2_reading, pd_vol3_reading, rfic_temp_reading);
	...
	else if(cmd[0] == 54)
	{
		...
		printk("RSSI (dBm) : %d.%d, %d.%d, %d.%d, %d.%d\n", rssi[0]/10, ABS(rssi[0])%10 , rssi[1]/10, ABS(rssi[1])%10 , rssi[2]/10, ABS(rssi[2])%10 , rssi[3]/10, ABS(rssi[3])%10);
		...
		printk("Get Test Mode = Ant_Sel: %d, MCS: %d, BW: %d, Pkt_Len: %d, Protocol: %d, BF: %d\n",
		...
		if (cmd[5] == 1) {
			printk("RX IQ Clipping detected!\n");
		}
		else if (cmd[5] == 2) {
			printk("RX IQ Clipping detected!\n");
			printk("RX IQ training signal too weak!\n");
		}
	}
...
static char *sp_olog = NULL;

static int
qdrv_command_dumpkcrash(struct device *dev, int argc, char *argv[])
{
	int i;
	char *cp = sp_olog;

	/* Avoid crash if sp_olog is NULL */
	if (!cp) {
		printk("No crash records\n");
		return 0;
	}
	if (cp[CONFIG_ARC_MUC_STACK_SIZE - 1] == QTN_SAFE_SRAM_MAGIC) {
		for (i = 0; i < CONFIG_ARC_MUC_STACK_SIZE; i++) {
			/* Only printables and newlines */
			if ((*cp != '\n') && ((*cp < 32) || (*cp > 127))) {
				printk(".");
			} else {
				printk("%c", *cp);
		...
		printk("No crash buffer present\n");
	}
	return  0;
}
...
{
	struct qdrv_cb *qcb;
	char *cp = (char *)CONFIG_ARC_MUC_STACK_INIT - CONFIG_ARC_MUC_STACK_SIZE;
	qcb = dev_get_drvdata(dev);

	/* Copy out any crash log, let the parse function deal with validity of the buffer */
	if (sp_olog == NULL) {
		sp_olog = kmalloc(CONFIG_ARC_MUC_STACK_SIZE, GFP_KERNEL);
		if (sp_olog) {
			memcpy(sp_olog, cp, CONFIG_ARC_MUC_STACK_SIZE);
		}
	}
	if (qcb->resources == 0) {
		/*
...
static void inline qdrv_control_set_br_isolate(struct device *dev, int val)
{
	...
	if (mac)
		qw = qdrv_control_wlan_get(mac);
	if (qw)
		qw->br_isolate = (uint8_t)!!val;
	else
		printk("%s: failed to set br_isolate\n", __FUNCTION__);
}
...
static void qdrv_control_allow_all_vlan(uint8_t vapid, int allow)
{
	struct shared_params *shared_param = qtn_mproc_sync_shared_params_get();
	uint8_t *vlan_bitmap = shared_param->vlan_bitmap_lhost;
	uint16_t vid;

	for (vid = 0; vid < VLAN_GROUP_ARRAY_LEN; vid++) {
		if (allow)
			qtn_vlan_allow(vlan_bitmap, vid, vapid);
		else
			qtn_vlan_disallow(vlan_bitmap, vid, vapid);
	}
}
...
static int qdrv_control_set_sta_vlan(struct qdrv_mac *mac, const char *addr, uint16_t vid)
{
	...
	qn = container_of(ni, struct qdrv_node, qn_node);
	qn->vlan_id = vid;

	/* notification to MuC & AuC */
	ic->ic_setparam(ni, IEEE80211_PARAM_SET_STA_VLAN, (int)vid, sta_addr, ETH_ALEN);
...
{
	int *vlan_enabled = qtn_mproc_sync_shared_params_get()->vlan_enabled_lhost;
	struct qdrv_wlan *qw;
	...
	if (*vlan_enabled != enable) {
		*vlan_enabled = enable;
		...
		ic = &qw->ic;
		qdrv_wlan_vlan_enable(ic, enable);
	}
}

static int qdrv_control_vlanmode_switch(
		struct qdrv_mac *mac, const char *dev_name, const char *mode, uint16_t vid)
{
	uint16_t new_mode;
	uint16_t command;
	struct qdrv_wlan *qw = qdrv_mac_get_wlan(mac);
	...
	if (qv->iv.iv_opmode == IEEE80211_M_WDS) {
		printk(KERN_ERR"VLAN configuration is not "
					"supported on WDS interfaces\n");
		return -EINVAL;
	}

	vapid = QDRV_WLANID_FROM_DEVID(qv->devid);

	new_mode = QVLAN_MODE_PTHRU;
	if (strcmp(mode, "bind") == 0) {
                new_mode = QVLAN_MODE_MBSS;
		command = QVLAN_CMD_BIND;
        } else if (strcmp(mode, "unbind") == 0) {
		command = QVLAN_CMD_UNBIND;
	} else if (strcmp(mode, "passthru") == 0) {
		if (vid == QVLAN_VID_ALL)
			command = QVLAN_CMD_PTHRU_ALL;
		else
			command = QVLAN_CMD_PTHRU;
	} else if (strcmp(mode, "unpassthru") == 0) {
		if (vid == QVLAN_VID_ALL)
			command = QVLAN_CMD_UNPTHRU_ALL;
		else
			command = QVLAN_CMD_UNPTHRU;
	} else if (strcmp(mode, "dynamic") == 0) {
		if (vid) {
			new_mode = QVLAN_MODE_DYNAMIC;
			command = QVLAN_CMD_DYNAMIC;
		} else {
			new_mode = QVLAN_MODE_PTHRU;
			command = QVLAN_CMD_UNDYNAMIC;
		}
	} else {
		printk(KERN_ERR"vlan command must be bind, unbind, passthru or unpassthru\n");
                return -EINVAL;
        }

	if (command == QVLAN_CMD_UNDYNAMIC && qv->qv_vlanmode != QVLAN_MODE_DYNAMIC)
		return 0;

	if (new_mode == QVLAN_MODE_DYNAMIC) {
		qdrv_control_vlan_enable(mac, 1);
	} else if (!*shared_param->vlan_enabled_lhost) {
		printk(KERN_ERR"VLAN functionality is disabled;command %u\n", command);
		return -EINVAL;
	}

	if (command == QVLAN_CMD_UNBIND && qv->qv_vlanmode != QVLAN_MODE_MBSS) {
		printk(KERN_ERR"QVLAN: config command \"unbind\" is rejected - %s not in %s\n", dev_name, QVLAN_MODE_STR_BIND);
		return -EINVAL;
	}

	if (vid >= VLAN_GROUP_ARRAY_LEN
			&& (vid != QVLAN_VID_ALL
			|| (command != QVLAN_CMD_PTHRU_ALL && command != QVLAN_CMD_UNPTHRU_ALL))) {
		printk(KERN_ERR"Invalid VLAN id %u\n", vid);
		return -EINVAL;
	}

	/*
	* hostlink to MuC and AuC, bind VID and MBSS together
	* arg2 def:
	* bit[0:11] -- vlan id
	* bit[19:16] -- bit[19:16] - VLAN command
	*/
	vlan_cfg = (vid & QVLAN_MASK_VID) |
			(((uint32_t)command) << QVLAN_SHIFT_MODE);
	ic->ic_setparam(qv->iv.iv_bss, IEEE80211_PARAM_VLAN_CONFIG, vlan_cfg, NULL, 0);

	qv->qv_vlan = (vid & QVLAN_MASK_VID) |
			(((uint32_t)new_mode) << QVLAN_SHIFT_MODE);
	qv->qv_vlanmode = new_mode;

	if (command == QVLAN_CMD_PTHRU) {
		qtn_vlan_allow(vlan_bitmap, vid, vapid);
	} else if (command == QVLAN_CMD_UNPTHRU) {
		qtn_vlan_disallow(vlan_bitmap, vid, vapid);
	} else if (command == QVLAN_CMD_PTHRU_ALL) {
		qdrv_control_allow_all_vlan(vapid, 1);
	} else if (command == QVLAN_CMD_UNPTHRU_ALL) {
		qdrv_control_allow_all_vlan(vapid, 0);
	}

	ret = qdrv_vap_vlan2index_sync(qv);
...
static int qdrv_command_set(struct device *dev, int argc, char *argv[])
{
	...
	if (strcmp(name, "vlan") == 0) {
		...
		if (argc == 5) {
			dev_name = argv[2];
			mode = argv[3];
			vid = simple_strtol(argv[4], NULL, 10);
		} else if (argc == 3) {
			return qdrv_control_set_vlan_enable((struct qdrv_mac *) (&qcb->macs[0]), argv[2]);
		} else {
			return -EINVAL;
		}
		return qdrv_control_vlanmode_switch((struct qdrv_mac *) (&qcb->macs[0]),
							dev_name, mode, vid);
	}
	...
	if(strcmp(name, "test1") == 0) {
		static struct pm_qos_request_list * test_mode_pm_overide = NULL;

		if (g_dbg_test_flags == 0x2) {
			if (test_mode_pm_overide == NULL) {
				test_mode_pm_overide = pm_qos_add_request(PM_QOS_POWER_SAVE,
						       BOARD_PM_LEVEL_FORCE_NO);
			}
			DBG_LOG_FUNC |= QDRV_LF_DFS_TESTMODE;
			DBG_LOG_LEVEL = DBG_LL_NOTICE;
		} else if (g_dbg_test_flags == 0x0) {
			if (test_mode_pm_overide != NULL) {
				pm_qos_remove_request(test_mode_pm_overide);
				test_mode_pm_overide = NULL;
			}
			DBG_LOG_FUNC &= ~QDRV_LF_DFS_TESTMODE;
	...
	} else if (strcmp(name, "br_vlan_forward") == 0) {
		br_vlan_set_forward(*((int*)dest));
	} else if(strcmp(name, "vlan_promisc") == 0) {
		br_vlan_set_promisc(*((int*)dest));
	} else if(strcmp(name, "br_isolate") == 0) {
		qdrv_control_set_br_isolate(dev, *((int*)dest));
...
qdrv_show_dsp_stats(struct seq_file *s, void *data, u32 num)
{
	...
	for (i = 0; i < ARRAY_SIZE(txbf_mbox->dsp_stats.dsp_act_rx); i++)
		seq_printf(s, "%-*s[%u] %u\n", QDRV_UC_STATS_DESC_LEN, "dsp_act_rx", i, txbf_mbox->dsp_stats.dsp_act_rx[i]);

	for (i = 0; i < ARRAY_SIZE(txbf_mbox->dsp_stats.dsp_act_rx_failed); i++)
		seq_printf(s, "%-*s[%u] %u\n", QDRV_UC_STATS_DESC_LEN, "dsp_act_rx_failed", i, txbf_mbox->dsp_stats.dsp_act_rx_failed[i]);

	for (i = 0; i < ARRAY_SIZE(txbf_mbox->dsp_stats.dsp_act_rx_inval_len); i++)
		seq_printf(s, "%-*s[%u] %u\n", QDRV_UC_STATS_DESC_LEN, "dsp_act_rx_inval_len", i, txbf_mbox->dsp_stats.dsp_act_rx_inval_len[i]);
	...
	seq_printf(s, "%-*s %u\n", QDRV_UC_STATS_DESC_LEN, "dsp_ipc_int_in", txbf_mbox->dsp_stats.dsp_ipc_int_in);
	seq_printf(s, "%-*s %u\n", QDRV_UC_STATS_DESC_LEN, "dsp_ipc_int_out", txbf_mbox->dsp_stats.dsp_ipc_int_out);
	...
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_act_rx_grp_sel", txbf_mbox->dsp_stats.dsp_act_rx_grp_sel);
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_act_rx_precoding", txbf_mbox->dsp_stats.dsp_act_rx_precoding);
	...
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_mu_grp_inst_success", txbf_mbox->dsp_stats.dsp_mu_grp_inst_success);
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_mu_rank_negative", txbf_mbox->dsp_stats.dsp_mu_rank_negative);
	...
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_mu_mu_node_del", txbf_mbox->dsp_stats.dsp_mu_mu_node_del);
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_mu_locked_cache_op", txbf_mbox->dsp_stats.dsp_mu_locked_cache_op);

	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_su_act_rx", txbf_mbox->dsp_stats.dsp_su_act_rx);
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_mu_act_rx", txbf_mbox->dsp_stats.dsp_mu_act_rx);
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_mimo_ctrl_fail", txbf_mbox->dsp_stats.dsp_mimo_ctrl_fail);
	seq_printf(s, "%-*s %d\n", QDRV_UC_STATS_DESC_LEN, "dsp_act_frm_bad", txbf_mbox->dsp_stats.dsp_act_frm_bad);
...
qdrv_control_show_vlan_config(struct seq_file *s, void *data, u32 num)
{
	...
	if (!ndev) {
		printk(KERN_ERR"%s is not a valid wireless interface to show vlan config\n", ifname);
		return;
	}
	qv = netdev_priv(ndev);
	dev_put(ndev);
	...
	vcfg = kzalloc(sizeof(struct qtn_vlan_config) + 1, GFP_KERNEL);
	if (!vcfg) {
	...
	/*
	* Encode vlan config info into a NULL-terminated string
	* So RPC call won't break because of \0
	*/
	vcfg->vlan_cfg = 0x80018001; /* Set 0,15,16,31 bits to make sure of no zero byte */
	vcfg->vlan_cfg |= (*shared_param->vlan_enabled_lhost ? (qv->qv_vlan << 1) : (0xffffffff << 1));
	if (((qv->qv_vlan & QVLAN_MASK_MODE) >> QVLAN_SHIFT_MODE) == QVLAN_MODE_PTHRU) {
		for (vid = 0; vid < ARRAY_SIZE(vcfg->vlan_bitmap); vid++)
			vcfg->vlan_bitmap[vid] = (uint8_t)BIT(7);

		for (vid = 0; vid < VLAN_GROUP_ARRAY_LEN; vid++) {
			if (qtn_vlan_is_allowed(vlan_bitmap, vid, vapid))
				vcfg->vlan_bitmap[vid / 7] |= BIT(vid % 7);
		}
	}

	ret = seq_write(s, vcfg, sizeof(struct qtn_vlan_config) + 1);
	if (ret)
...
mu_exit:
	if (res == -1) {
		printk("Usage: mu {set | get | clr | dbg} [sub-cmd] [mac_addr] [options]\n");
	}
