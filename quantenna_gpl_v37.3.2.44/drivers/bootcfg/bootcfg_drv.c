...
static int bootcfg_store_read(void)
{
	...
	if (ret < 0 && ret != -ENODATA) {
		...
		/* Check CRC */
		crc = bootcfg_crc32(0, gBootCfgData.data, gBootCfgData.size);
		if (crc != gBootCfgData.crc) {
			crc = bootcfg_crc32(0, gBootCfgData.data, (BOOT_CFG_BASE_SIZE - sizeof(u32)) );
			if (crc != gBootCfgData.crc) {
				printk(KERN_WARNING "%s %s: crc32 does not match crc: %x expect %x\n",
					DRV_NAME, __FUNCTION__, crc, gBootCfgData.crc);
			}
			else {
				gBootCfgData.size = BOOT_CFG_BASE_SIZE - sizeof(u32);
			}
		}
...
int bootcfg_create(const char *filename, u32 size)
{
	...
	if (size != 0) {
		...
		// create our entry
		if ((size + addr) > BOOT_CFG_DATA_SIZE) {
		   ...
		   unsigned long count, void *data)
{
	...
	if ((file->f_pos + count) > id->len) {
		...
		if (id->addr == 0) {
			...
			if (id->addr + len > BOOT_CFG_DATA_SIZE) {
				/* can't increase up to desired size */
				printk("bootcfg error: file too large\n");
				goto bail;
			}
			...
			if ((addr + len) > BOOT_CFG_DATA_SIZE) {
				printk("bootcfg error: file too large\n");
