...
typedef uint32_t (*fwt_fetch_timestamp)(void *ctx, uint16_t index);
typedef int (*fwt_notify_entry_add)(void *ctx, uint16_t index);
void fwt_sw_register_qdrv_cb(void *ctx,
		fwt_fetch_timestamp fetch, fwt_notify_entry_add notify);
