 ...
 * @param index: the HW FWT index that matches the SW one
...
/*
 * Update existing entry's SW timestamp
 * @param index: the HW FWT index that matches the SW one
 */
int fwt_db_update_timestamp(uint16_t index);

/*
 * get existing entry's SW timestamp
 * @param index: the HW FWT index that matchs the SW one
 */
