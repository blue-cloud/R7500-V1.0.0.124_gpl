 ...
 *
 * Piggy: SRAM: [text, data, bss, <---- gap -----> stack]
 * Decompresses u-boot-full into gap, jumps to gap.
 * Full:  SRAM: [heap, stack, text, data, bss, <--unused-->, muc_stack]
 */
#define CONFIG_ARC_STAGE2_OFFSET	(0x14000)

#define CONFIG_ARC_UBOOT_BEGIN		RUBY_SRAM_BEGIN
#define CONFIG_ARC_UBOOT_SIZE_LIMIT	0x40000	/* 256k to maintain Ruby support */
...
#define CONFIG_ARC_TEXT_SIZE		(CONFIG_ARC_UBOOT_SIZE_LIMIT - CONFIG_ARC_TEXT_OFFSET - 4 - CONFIG_ARC_MUC_STACK_SIZE)
#define CONFIG_ARC_TEXT_END		(CONFIG_ARC_TEXT_BEGIN + CONFIG_ARC_TEXT_SIZE)
#define CONFIG_ARC_STACK_BEGIN		(CONFIG_ARC_TEXT_BEGIN - 4)
#define CONFIG_ARC_STACK_SIZE		(8*1024)
#define CONFIG_ARC_HEAP_SIZE		(CONFIG_ARC_TEXT_OFFSET - CONFIG_ARC_STACK_SIZE)
...
/* Ethernet */
	...
	#else
		#define CONFIG_BOOTFILE		"ruby-linux.lzma.img"
	#endif /* #ifdef TOPAZ_PLATFORM */
	...
	#elif RUBY_FPGA_PLATFORM

		#define CONFIG_BOOTARGS \
			"console=ttyS0," \
			MK_STR(RUBY_SERIAL_BAUD) \
			"n8 earlyprintk=1 hw_config_id=" \
			MK_STR(DEFAULT_BOARD_ID) \
			" spi=" MK_STR(CONFIG_NOSPI) \
			" wmac_mode=" MK_STR(CONFIG_WMAC_MODE)
