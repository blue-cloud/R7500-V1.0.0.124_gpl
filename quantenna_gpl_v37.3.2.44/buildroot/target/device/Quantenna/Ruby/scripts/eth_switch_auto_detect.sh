...
emac1_check_for_auto_detect() {
	...
	do
		...
		if [ "$port_connected" != "" ]; then
			...
			if [ $emac1_yes -ge 3 ]; then
				break;
			fi
		else
			emac1_no=$((emac1_no+1))
			if [ $emac1_no -ge 3 ]; then
				break;
			fi
		fi
	done
	echo "emac1_yes is $emac1_yes, emac1_no is $emac1_no"
}

dhcp_request_for_auto_detect() {
	dhclient -4 $eth1_port &
	...
	if [ $dhcp_response == 0 ]; then
		...
		ifconfig $eth1_port 0.0.0.0 up
		kill `cat /var/lib/dhcp/dhclient.pid`
		rm -f /var/lib/dhcp/resolv.conf
	fi
