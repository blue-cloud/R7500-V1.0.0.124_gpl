...
def parse_command_line():
    global ca_address, server_address
...
def handle_incoming_connection():
    ...
    try:
        ...
        logging.error("can't connect to native control, error: %s", message)
        ca_socket.close()
        ca_socket = None
...
def shutdown_broken_socket(broken_socket):
    logging.info("closing socket %s", broken_socket.getsockname())
    ...
    connections_from_ucc.remove(broken_socket)
    broken_socket.close()
