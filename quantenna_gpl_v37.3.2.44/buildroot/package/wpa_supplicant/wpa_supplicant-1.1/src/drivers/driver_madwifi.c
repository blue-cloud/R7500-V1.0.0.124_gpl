...
#define CMD_CREATE_AP_INTF	"/scripts/repeater.sh create_ap_intf"
#define CMD_DESTROY_AP_INTF	"/scripts/repeater.sh destroy_ap_intf"
static int wpa_driver_madwifi_start_repeater_ap(void *priv)
{
	static const char *argv[] = {"hostapd", "-B", "/mnt/jffs2/hostapd.conf", "-P", "/tmp/hostapd.pid", NULL};
	int ret;
	pid_t pid;

	ret = system(CMD_CREATE_AP_INTF);
	if (ret == -1 || WEXITSTATUS(ret) != 0) {
		wpa_printf(MSG_DEBUG, "Failed to create repeater AP interface\n");
		return ret;
	}
	...
	if (pid == 0) {
		...
		waitpid(pid, NULL, 0);
		wpa_driver_madwifi_find_pid(priv);
	} else {
		ret = -errno;
		wpa_printf(MSG_DEBUG, "Failed to start hostapd\n");
		system(CMD_DESTROY_AP_INTF);
	}

	return ret;
}

static void wpa_driver_madwifi_stop_repeater_ap(void *priv)
{
	struct wpa_driver_madwifi_data *drv = priv;
	static unsigned int max_loop = 300;
	int ret;
	unsigned int cnt = 0;

	if (drv->hostapd_pid == 0)
		return;

	kill(drv->hostapd_pid, SIGTERM);

	while (cnt++ < max_loop) {
		os_sleep(0, 100000);
		ret = kill(drv->hostapd_pid, SIGTERM);
		if (ret < 0 && errno == ESRCH)
			break;
	}

	if (cnt == max_loop) {
		wpa_printf(MSG_DEBUG, "Failed to stop hostapd and AP interface\n");
		return;
	}
...
const struct wpa_driver_ops wpa_driver_madwifi_ops = {
	...
	.start_repeater_ap	= wpa_driver_madwifi_start_repeater_ap,
	.stop_repeater_ap	= wpa_driver_madwifi_stop_repeater_ap,
#ifdef CONFIG_TDLS
