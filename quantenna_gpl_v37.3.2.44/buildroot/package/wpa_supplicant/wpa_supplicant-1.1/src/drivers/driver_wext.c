...
wpa_driver_wext_event_wireless_custom(void *ctx, char *custom)
{
	...
	if (os_strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
		...
		case WDS_EXT_RECEIVED_MBS_IE:
			...
			while (ssid) {
				if (ssid->disabled == 0 && ssid->ssid_len > 0 &&
						strncmp(ssid->ssid, event_data->ssid,
						ssid->ssid_len) == 0)
					break;
				ssid = ssid->next;
			}
			if (ssid && ssid->key_mgmt != WPA_KEY_MGMT_NONE && ssid->psk_md5) {
				wpa_driver_wext_generate_wds_key(event_data->mac,
						wpa_s->own_addr, ssid->psk_md5, wds_key);
				wpa_snprintf_hex(wds_key_hex, sizeof(wds_key_hex),
						wds_key, sizeof(wds_key));
			} else {
				snprintf(wds_key_hex, sizeof(wds_key_hex), "NULL");
			}
			snprintf(cmd, sizeof(cmd), "%s %s peer=" MACSTR " wds_key=%s channel=%d bw=%d &",
				QTN_WDS_EXT_SCRIPT,
				"START-AP-RBS",
				MAC2STR(event_data->mac),
				wds_key_hex,
				event_data->channel, event_data->bandwidth);
			break;
		...
		if (ok_to_run) {
			wpa_printf(MSG_DEBUG, "WEXT: call command - %s", cmd);
			system(cmd);
		}
	}
}
