...
static inline int wpa_drv_start_repeater_ap(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->start_repeater_ap)
		return wpa_s->driver->start_repeater_ap(wpa_s->drv_priv);

	return -1;
}

static inline void wpa_drv_stop_repeater_ap(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->stop_repeater_ap)
		wpa_s->driver->stop_repeater_ap(wpa_s->drv_priv);
}
