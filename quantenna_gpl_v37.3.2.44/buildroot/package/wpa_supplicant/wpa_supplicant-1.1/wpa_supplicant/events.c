				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->global->repeater_mode && wpa_s->key_mgmt == WPA_KEY_MGMT_NONE)
		wpa_drv_start_repeater_ap(wpa_s);
}
					  ...
					  u16 reason_code)
{
	...
	if (wpa_s->global->repeater_mode)
		wpa_drv_stop_repeater_ap(wpa_s);
}
