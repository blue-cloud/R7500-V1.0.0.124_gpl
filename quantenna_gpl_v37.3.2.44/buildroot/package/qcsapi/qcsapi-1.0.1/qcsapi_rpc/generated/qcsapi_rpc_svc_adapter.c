...
bool_t qcsapi_wifi_scs_switch_channel_remote_1_svc(qcsapi_wifi_scs_switch_channel_rpcdata *__req, qcsapi_wifi_scs_switch_channel_rpcdata *__resp, struct svc_req *rqstp)
{
	...
	__resp->return_code = qcsapi_wifi_scs_switch_channel(ifname);
...
bool_t qcsapi_config_update_parameter_remote_1_svc(qcsapi_config_update_parameter_rpcdata *__req, qcsapi_config_update_parameter_rpcdata *__resp, struct svc_req *rqstp)
{
	...
	if (__resp->return_code >=0 ) {
		if (strstr(param_name, "bf")) {
			if (strstr(param_value, "1")) {
				system("bfon");
				printf("beamforming on\n");
			} else {
				system("bfoff");
				printf("beamforming off\n");
			}
		}
	}
...
bool_t qcsapi_wifi_get_bss_assoc_limit_remote_1_svc(qcsapi_wifi_get_bss_assoc_limit_rpcdata *__req, qcsapi_wifi_get_bss_assoc_limit_rpcdata *__resp, struct svc_req *rqstp)
{
	...
	__resp->return_code = qcsapi_wifi_get_bss_assoc_limit(ifname, __req->p_bss_lim_pri);

	__resp->p_bss_lim_pri = __rpc_prepare_data(__req->p_bss_lim_pri, sizeof(*__resp->p_bss_lim_pri));
	if (debug) { fprintf(stderr, "%s:%d %s post\n", __FILE__, __LINE__, __FUNCTION__); }
...
bool_t qcsapi_wifi_set_bss_assoc_limit_remote_1_svc(qcsapi_wifi_set_bss_assoc_limit_rpcdata *__req, qcsapi_wifi_set_bss_assoc_limit_rpcdata *__resp, struct svc_req *rqstp)
{
	...
	__resp->return_code = qcsapi_wifi_set_bss_assoc_limit(ifname, __req->bss_lim_pri);
...
{
	char * ifname = (__req->ifname == NULL) ? NULL : __req->ifname->data;
	char * group_key_interval = (__req->group_key_interval == NULL) ? NULL : arg_alloc();

	memset(__resp, 0, sizeof(*__resp));
	if (debug) { fprintf(stderr, "%s:%d %s pre\n", __FILE__, __LINE__, __FUNCTION__); }

	__resp->return_code = qcsapi_wifi_get_group_key_interval(ifname, group_key_interval);

	if (group_key_interval) {
		__resp->group_key_interval = malloc(sizeof(*__resp->group_key_interval));
		__resp->group_key_interval->data = group_key_interval;
	}
	if (debug) { fprintf(stderr, "%s:%d %s post\n", __FILE__, __LINE__, __FUNCTION__); }
...
{
	char * ifname = (__req->ifname == NULL) ? NULL : __req->ifname->data;
	char * group_key_interval = (__req->group_key_interval == NULL) ? NULL : __req->group_key_interval->data;

	memset(__resp, 0, sizeof(*__resp));
	if (debug) { fprintf(stderr, "%s:%d %s pre\n", __FILE__, __LINE__, __FUNCTION__); }

	__resp->return_code = qcsapi_wifi_set_group_key_interval(ifname, group_key_interval);
...
bool_t qcsapi_wifi_vlan_config_remote_1_svc(qcsapi_wifi_vlan_config_rpcdata *__req, qcsapi_wifi_vlan_config_rpcdata *__resp, struct svc_req *rqstp)
{
	...
	__resp->return_code = qcsapi_wifi_vlan_config(ifname, cmd, __req->vlanid, __req->flags);
...
{
	char * ifname = (__req->ifname == NULL) ? NULL : __req->ifname->data;
	char * vcfg = (__req->vcfg == NULL) ? NULL : arg_alloc();

	memset(__resp, 0, sizeof(*__resp));
	if (debug) { fprintf(stderr, "%s:%d %s pre\n", __FILE__, __LINE__, __FUNCTION__); }

	__resp->return_code = qcsapi_wifi_show_vlan_config(ifname, vcfg);

	if (vcfg) {
		__resp->vcfg = malloc(sizeof(*__resp->vcfg));
		__resp->vcfg->data = vcfg;
	}
	if (debug) { fprintf(stderr, "%s:%d %s post\n", __FILE__, __LINE__, __FUNCTION__); }
...
bool_t qcsapi_enable_vlan_pass_through_remote_1_svc(qcsapi_enable_vlan_pass_through_rpcdata *__req, qcsapi_enable_vlan_pass_through_rpcdata *__resp, struct svc_req *rqstp)
{
	char * ifname = (__req->ifname == NULL) ? NULL : __req->ifname->data;
	...
	__resp->return_code = qcsapi_enable_vlan_pass_through(ifname, __req->enabled);
...
bool_t qcsapi_wifi_get_node_param_remote_1_svc(qcsapi_wifi_get_node_param_rpcdata *__req, qcsapi_wifi_get_node_param_rpcdata *__resp, struct svc_req *rqstp)
{
	...
	char * input_param_str = (__req->input_param_str == NULL) ? NULL : arg_alloc();

	memset(__resp, 0, sizeof(*__resp));
	if (debug) { fprintf(stderr, "%s:%d %s pre\n", __FILE__, __LINE__, __FUNCTION__); }

	__resp->return_code = qcsapi_wifi_get_node_param(ifname, __req->node_index, param_type, __req->local_remote_flag, input_param_str, (qcsapi_measure_report_result *)__req->report_result);

	if (input_param_str) {
		__resp->input_param_str = malloc(sizeof(*__resp->input_param_str));
		__resp->input_param_str->data = input_param_str;
	}
