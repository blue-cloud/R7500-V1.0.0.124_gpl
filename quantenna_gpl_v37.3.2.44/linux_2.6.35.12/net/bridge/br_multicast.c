...
static void br_igmp_snoop_trace_v3_report(struct net_device *dev, __be32 saddr,
	const struct igmphdr *igmp, const char *vlan_str)
{
	...
	BR_SNOOP_DBG("IGMPv3 Membership Report from " NIPQUAD_FMT ", vlan %s\n", NIPQUAD(saddr), vlan_str);
	...
	for (grec_idx = 0; grec_idx < ngrec; grec_idx++) {
		...
		BR_SNOOP_DBG("  %u " NIPQUAD_FMT " %s ",
			grec_idx,
			NIPQUAD(igmpv3_grec->grec_mca),
			igmpv3_grec_type[igmpv3_grec->grec_type - 1]);

		/* dump source list */
		for (nsrc_idx = 0; nsrc_idx < grec_nsrcs; nsrc_idx++) {
			BR_SNOOP_DBG("%s" NIPQUAD_FMT, is_first ? "" : ", ",
				NIPQUAD(igmpv3_grec->grec_src[nsrc_idx]));
			is_first = 0;
		}
		BR_SNOOP_DBG("\n");
...
static void br_igmp_snoop_trace(struct net_device *dev, __be32 saddr,
	const struct igmphdr *igmp, struct sk_buff *skb)
{
	...
	case IGMPV3_HOST_MEMBERSHIP_REPORT:
		br_igmp_snoop_trace_v3_report(dev, saddr, igmp, vlan_str);
		return;
	default:
	...
	BR_SNOOP_DBG(" from " NIPQUAD_FMT " group " NIPQUAD_FMT "\n", NIPQUAD(saddr),
		NIPQUAD(igmp->group));
}
...
static void br_multicast_port_group_expired(unsigned long data)
{
	...
	if (pg->addr.proto == htons(ETH_P_IP))
		BR_SNOOP_DBG("Member port timeout: group " NIPQUAD_FMT ", subport 0x%x\n",
			NIPQUAD(pg->addr.u.ip4), pg->sub_port);
				 ...
				 struct sk_buff *skb)
{
	...
	br_igmp_snoop_trace(skb->dev, iph->saddr, ih, skb2);
